...
class TokenMiddleware( object ):
    ...
    def process_request(self, request):
        ...
        if i and token and timestamp:
            # we have been provided a token. confirm that this is coming from hs
            if hashlib.sha1(i + timestamp + HOOTSUITE_SSO_SHARED_SECRET).hexdigest() != token:
                logger.error('Request token dont match. Returning HttpResponseForbidden')
...
#            logger.error('Request token dont match. Returning ')
#            return HttpResponseForbidden()
